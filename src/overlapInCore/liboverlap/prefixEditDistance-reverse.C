...
prefixEditDistance::reverse(char    *A,   int32 m,
...
#ifdef SHOW_BRI
  fprintf(stderr, "REVERSE errorlimit %d m %d n %d\n", Error_Limit, m, n);
#endif
  ...
  if  (Row == m) {
    ...
    Match_To_End = true;
#ifdef SHOW_BRI
    // fprintf(stderr, "WorkArea %2d REV exact match\n", omp_get_thread_num());
#endif
    return  0;
  }
  ...
  for  (e = 1;  e <= Error_Limit;  e++) {
#ifdef SHOW_BRI
    fprintf(stderr, "REVERSE errors %d\n", e);
#endif
    ...
    for  (d = Left;  d <= Right;  d++) {
...
#ifdef SHOW_BRI
      fprintf(stderr, "Edit_Array_Lazy[%d][%d] = %d -- min %d\n", e, d, Row, minMatchLen(e, d, m));
#endif
      ...
      if  (Row == m || Row + d == n) {
        ...
        // fprintf(stderr, "WorkArea %2d REV e=%d MIN=%d Tail_Len=%d Max_Score=%f Score=%f slope=%f SLOPE=%f\n",
...
#ifdef SHOW_BRI
      fprintf(stderr, "EAL[e][left=%d] %d  DELETE r1\n", Left, Edit_Array_Lazy[e][Left]);
#endif
      Left++;
    }
...
#ifdef SHOW_BRI
        fprintf(stderr, "EAL[e][left=%d] %d  DELETE r2\n", Left, Edit_Array_Lazy[e][Left]);
#endif
        Left++;
      }
    ...
    if  (Left > Right) {
#ifdef SHOW_EXTEND_ALIGN
      // fprintf(stderr, "WorkArea %2d REV BREAK at Left=%d Right=%d\n", omp_get_thread_num(), Left, Right);
#endif
      break;
    }
...
#ifdef SHOW_BRI
      fprintf(stderr, "EAL[e][right=%d] %d  DELETE r3\n", Right, Edit_Array_Lazy[e][Right]);
#endif
      Right--;
    }
...
#ifdef SHOW_BRI
        fprintf(stderr, "EAL[e][right=%d] %d  DELETE r4\n", Right, Edit_Array_Lazy[e][Right]);
#endif
        Right--;
      }
    ...
    //  Save the longest alignment found for errors e
    for  (d = Left;  d <= Right;  d++)
...
#ifdef SHOW_EXTEND_ALIGN
  // fprintf(stderr, "WorkArea %2d REV ERROR_LIMIT at e=%d Error_Limit=%d best_e=%d\n", omp_get_thread_num(), e, Error_Limit, Max_Score_Best_e);
#endif
